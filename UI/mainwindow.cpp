...
void MainWindow::updateInfo(fromServer * s, scoreToClient* scores, dispatchStruct *d)
{
    ...
    if (d->type == STRUCTDOWN)
    {
        if (strcmp(d->name,"down")==0){
            ...
            QString sc("<center> Un autre joueur a trouvé le trésor et a changé d'étage. <br/> Vous allez, vous aussi passer à l'étage inférieur <br/> Il est temps de faire un point sur les scores: <br/>");
